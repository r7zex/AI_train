...
function findCurrentBlock(activeTopicId?: string) {
  for (const block of flowCourseBlocks) {
    if (block.subblocks.some((subblock) => subblock.themes.some((topic) => topic.id === activeTopicId))) {
      return block.id
    }
  }
  return ''
...
export default function CourseSidebar({ activeTopicId, progress, getTopicProgress, getBlockProgress }: CourseSidebarProps) {
  ...
  return (
    <aside className="sticky top-[42px] hidden h-[calc(100vh-42px)] w-[216px] shrink-0 self-start overflow-y-auto bg-[#202020] text-white lg:block">
