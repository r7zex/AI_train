  ...
  if (!topic || !section) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <Link to="/topics" className="text-blue-600 hover:underline">← Вернуться к списку тем</Link>
      </div>
    )
  }
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        {ContentComponent ? (
          ...
          <div className="py-20 text-center text-gray-500">
            ...
            <p>Контент для этой темы не найден.</p>
          </div>
        )}
      </div>

      {/* Navigation between topics in section */}
      <div className="mt-8 flex justify-between">
        {(() => {
          ...
          return (
            <>
              {prev ? (
                <Link to={`/topics/${prev.id}`} className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 bg-white border border-gray-200 px-4 py-2 rounded-xl hover:shadow-sm transition-all">
                  ← {prev.title}
                </Link>
              ) : <div />}
              {next ? (
                <Link to={`/topics/${next.id}`} className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 bg-white border border-gray-200 px-4 py-2 rounded-xl hover:shadow-sm transition-all">
                  {next.title} →
                </Link>
              ) : <div />}
            </>
