...
const tables: ComparisonTable[] = [
  ...
  {
    id: 'norm-variants',
    title: 'BatchNorm vs LayerNorm vs InstanceNorm vs GroupNorm vs WeightNorm',
    icon: '⚖️',
    color: 'pink',
    description: 'Пять вариантов нормализации: по каким осям нормализуют, зависимость от батча, параметры и область применения.',
    columns: [
      { label: 'Критерий' },
      { label: 'BatchNorm' },
      { label: 'LayerNorm' },
    ...
    rows: [
      { header: 'Оси нормализации', values: ['Батч + пространство (N, H, W)', 'Каналы/признаки (C)', 'Пространство (H, W)', 'Группы каналов (C/G)', 'Веса слоя (не активации)'] },
      { header: 'Зависимость от батча', values: ['✅ Да', '❌ Нет', '❌ Нет', '❌ Нет', '❌ Нет'] },
      { header: 'Применение', values: ['CNN (ResNet, VGG)', 'Transformer, RNN', 'Style Transfer, GAN', 'Малые батчи, видео', 'RNN, policy networks, RL'] },
      { header: 'Параметры', values: ['γ,β по каналу', 'γ,β по признаку', 'γ,β по каналу', 'γ,β по каналу/группе', 'g (скаляр), v (вектор)'] },
      { header: 'При инференсе', values: ['Running mean/var', 'Вычисляется на лету', 'Вычисляется на лету', 'Вычисляется на лету', 'Перепараметризует w=g·v/‖v‖'] },
      { header: 'Плюсы', values: ['Ускоряет сходимость', 'Не зависит от батча', 'Стиль на уровне экземпляра', 'Компромисс BN/LN', 'Ускоряет обучение, прост'] },
      { header: 'Минусы', values: ['Плохо при малом батче', 'Хуже для CNN', 'Не подходит классификации', 'Нужно подобрать G', 'Не центрирует активации'] },
    ],
  },
]
