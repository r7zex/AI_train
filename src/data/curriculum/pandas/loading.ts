...
export const topicPandasReadInspect = pandasTopic(
  ...
  [
    theoryStep(
      'pandas-read-eda-checklist',
      'Первый EDA-чеклист',
      'Нельзя сразу обучать модель после `read_csv`.',
      [
        section('checklist', 'От CSV к пониманию таблицы', [
          'После `pd.read_csv()` нельзя сразу обучать модель: сначала нужно понять структуру таблицы. Сколько строк, какие столбцы, какие типы, есть ли пропуски, какие значения повторяются и выглядят ли числовые диапазоны правдоподобно.',
          'Рабочий порядок первого осмотра обычно такой: `head -> shape -> info -> describe -> isna().sum() -> nunique`. Он не заменяет анализ, но быстро показывает, где таблица требует внимания.',
        ], {
          bullets: [
            '`head()` - увидеть реальные строки;',
            '`shape` - проверить размер;',
            '`info()` - типы и непустые значения;',
            '`describe()` - числовые диапазоны;',
            '`isna().sum()` - пропуски;',
            '`nunique()` - число уникальных значений.',
          ],
          callouts: [
            callout('Где это используется в EDA', 'Первый осмотр CSV, поиск проблем чтения, проверка пропусков и типов перед очисткой и выделением `X`/`y`.', 'example'),
          ],
        }),
      ],
    ),
    ...
    theoryStep(
      'pandas-read-checkpoint',
      'Что теперь умеем',
      'Собираем первый осмотр таблицы в один рабочий сценарий.',
      [
        section('checkpoint', 'Промежуточный вывод', [
          'Теперь загрузка CSV не заканчивается на `read_csv`. После чтения мы умеем быстро проверить строки, размер, названия столбцов, типы, непустые значения и числовую статистику.',
          'Эта связка нужна перед очисткой: если типы или пропуски выглядят странно, следующие темы pandas покажут, как выбирать столбцы, фильтровать строки и исправлять данные.',
        ], {
          callouts: [
            callout('Типичная ошибка', 'Считать, что `df.head()` достаточно для проверки данных. Первые строки могут выглядеть нормально, но пропуски, неверные типы и выбросы часто видны только через `info()`, `describe()` и проверки пропусков.', 'important'),
          ],
        }),
      ],
    ),
