...
export const topicPandasSelection = pandasTopic(
  ...
  [
    theoryStep(
      'pandas-selection-scenarios',
      'Зачем выбирать строки и столбцы',
      'Выбор данных связывает таблицу с задачей анализа.',
      [
        section('scenarios', 'Четыре частых сценария', [
          'В pandas выбор нужен не ради синтаксиса. Обычно мы выбираем target, выбираем features, оставляем строки по условию или собираем маленькую подтаблицу для анализа.',
          'Например, `y = df["price"]` берёт целевой столбец, `X = df[["area", "rooms"]]` берёт признаки, а `df[df["price"] > 10_000_000]` оставляет строки для анализа дорогих объектов.',
        ], {
          bullets: [
            'выбрать target;',
            'выбрать features;',
            'выбрать строки по условию;',
            'выбрать подтаблицу для анализа.',
          ],
        }),
      ],
    ),
    ...
    theoryStep(
      ...
      'Что теперь умеем',
      'Фиксируем выбор данных как подготовку к EDA и ML.',
      [
        section('checkpoint', 'Промежуточный вывод', [
          'Теперь мы можем не просто смотреть всю таблицу, а задавать к ней точные вопросы: какой столбец является target, какие столбцы пойдут в features, какие строки относятся к нужному сегменту.',
          'Это естественный мост к следующей теме: после выбора строк и столбцов логично научиться фильтровать, сортировать и создавать новые признаки.',
        ], {
          callouts: [
            callout('Типичная ошибка', '`df["col"]` возвращает `Series`, а `df[["col"]]` возвращает `DataFrame`. Для матрицы признаков `X` чаще нужен двумерный `DataFrame`.', 'important'),
          ],
        }),
      ],
    ),
