...
const functionReturnDescriptions: Record<string, string> = {
  ...
  'df.head()': 'возвращает первые строки таблицы',
  'df.tail()': 'возвращает последние строки таблицы',
  'df.sample()': 'возвращает случайные строки таблицы',
  'df.shape': 'возвращает размер таблицы: число строк и столбцов',
  'df.columns': 'возвращает названия столбцов',
  'df.dtypes': 'возвращает тип данных каждого столбца',
  'df.info()': 'печатает краткую сводку по столбцам, типам и непустым значениям',
  'df.describe()': 'возвращает статистическую сводку числовых столбцов',
  'df["column"]': 'возвращает один столбец как Series',
  'df[["col1", "col2"]]': 'возвращает несколько столбцов как DataFrame',
  'df.loc[]': 'выбирает строки и столбцы по меткам',
  'df.iloc[]': 'выбирает строки и столбцы по позициям',
  'df.sort_values()': 'возвращает таблицу, отсортированную по значениям столбца',
  'ascending=False': 'задаёт сортировку по убыванию',
  'df.isna()': 'показывает пропуски как таблицу True/False',
  'df.isna().sum()': 'считает пропуски по каждому столбцу',
  'df.dropna()': 'возвращает таблицу без строк или столбцов с пропусками',
  'df.fillna()': 'заменяет пропуски выбранным значением',
  'median() для чисел': 'возвращает медиану числового столбца для заполнения пропусков',
  'mode() для категорий': 'возвращает самое частое значение для заполнения пропусков',
  'df.duplicated()': 'возвращает маску повторяющихся строк',
  'df.drop_duplicates()': 'возвращает таблицу без дубликатов',
  'value_counts()': 'считает частоты значений в столбце',
  'groupby().mean()': 'группирует строки и считает среднее внутри каждой группы',
  'agg()': 'считает одну или несколько статистик по группам',
  'pivot_table()': 'строит сводную таблицу с агрегатами',
  'astype()': 'возвращает значения столбца в новом типе данных',
  'pd.get_dummies()': 'кодирует категориальные столбцы в 0/1-признаки',
}
