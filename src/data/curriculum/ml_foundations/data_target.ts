    ...
    theoryStep(
      ...
      [
        ...
        section('column-translations', 'Перевод названий столбцов', [
          '`client_id` — идентификатор клиента; `household_group` — идентификатор семьи: он уникален для каждой семьи и одинаков у всех её членов; `days_since_login` — число дней с последнего входа; `tariff` — тариф; `support_tickets` — количество обращений в поддержку; `churn_after_30d` — уйдёт ли клиент в течение следующих 30 дней.',
        ]),
        ...
        section('service-columns', 'Идентификаторы и служебные столбцы — не признаки', [
          '`client_id` только отличает одну запись от другой. `household_group` — идентификатор семьи: он уникален для семьи и одинаков у всех клиентов из этой семьи. Здесь он нужен, чтобы при проверке модели не разделить членов одной семьи между обучающей и тестовой выборками.',
          '`household_group` считается служебным именно в этом примере: значения `H1`, `H2` и `H3` — условные названия конкретных семей, а не характеристики клиента. Если передать их модели, она сможет запоминать семьи вместо поиска общей закономерности. Поэтому перед формированием `X` оставляют только характеристики, по которым модель должна строить прогноз.',
        ], {
          callouts: [
