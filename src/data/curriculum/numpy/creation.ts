...
export const topicNumpyCreation = numpyTopic(
  ...
  [
    theoryStep(
      'numpy-array-creation-intro',
      'Зачем сначала создавать массив',
      'Создание массива - первый шаг перед вычислениями.',
      [
        section('intro', 'Данные уже есть или нужна заготовка', [
          'Создание массива - это первый шаг перед вычислениями: либо мы переводим готовые данные в `ndarray`, либо создаём заготовку нужной формы, либо строим числовую сетку для эксперимента.',
          'Например, перед анализом цен квартир можно взять список цен и превратить его в массив через `np.array()`. Если заранее нужен вектор для накопления ошибок, удобнее создать нули через `np.zeros()`. Если нужен диапазон порогов для проверки модели, подойдут `np.arange()` или `np.linspace()`.',
        ], {
          callouts: [
            callout('Как выбрать функцию', 'готовые данные -> `np.array`; заготовка из нулей -> `np.zeros`; равномерная сетка -> `np.linspace`; диапазон с шагом -> `np.arange`.', 'schema'),
          ],
        }),
      ],
    ),
    ...
    theoryStep(
      'numpy-array-creation-usage',
      'Где это используется в ML/EDA',
      'Связываем функции создания с рабочими задачами.',
      [
        section('usage', 'От входных данных к эксперименту', [
          '`np.array()` обычно появляется, когда данные уже пришли из списка, теста или маленького учебного примера. `np.zeros()`, `np.ones()` и `np.full()` помогают подготовить массивы под будущие вычисления: счётчики, baseline-предсказания, накопители метрик.',
          '`np.arange()` удобен, когда важен шаг, например перебрать пороги 0, 5, 10. `np.linspace()` удобен, когда важно ровно N точек: сетка для графика, набор коэффициентов или учебный диапазон значений.',
        ], {
          bullets: [
            'переводить готовые данные в `ndarray`;',
            'создавать заготовки заданной формы;',
            'выбирать между шагом и количеством точек;',
            'не использовать `np.empty()` как учебную заготовку без явной причины.',
          ],
          callouts: [
            callout('Типичная ошибка', 'Путать `arange` и `linspace`: первая функция управляет шагом, вторая - количеством точек. Для ровно 50 точек надёжнее `np.linspace(...)`.', 'important'),
          ],
        }),
      ],
    ),
    quizStep(
      ...
      singleQuiz(
        ...
        'numpy-array-creation',
        'numpy-ml',
        'Какую функцию лучше использовать, если нужны ровно 5 равномерных точек от 0 до 1 включительно?',
        [
          { id: 'a', text: '`np.arange(0, 1, 5)`, потому что третий аргумент задаёт количество точек' },
          { id: 'b', text: '`np.arange(5) / 5`, потому что так всегда включается правая граница' },
          { id: 'c', text: '`np.linspace(0, 1, 5)`, потому что `num` задаёт количество точек' },
          { id: 'd', text: '`np.full(5, 1)`, потому что нужен диапазон от 0 до 1' },
        ],
        'c',
    ...
    quizStep(
      ...
      singleQuiz(
        ...
        'numpy-array-creation',
        'numpy-ml',
        'Что вернёт `np.arange(0, 6, 2)` и почему?',
        [
          { id: 'a', text: '`[0 2 4]`, потому что `stop` не включается' },
          { id: 'b', text: '`[0 2 4 6]`, потому что `stop` всегда включается' },
          { id: 'c', text: '`[2 4 6]`, потому что `start` задаёт только шаг' },
          { id: 'd', text: '`[0 1 2 3 4 5]`, потому что третий аргумент задаёт количество элементов' },
        ],
        'a',
