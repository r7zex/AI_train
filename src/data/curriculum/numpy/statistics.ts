...
export const topicNumpyAggregations = numpyTopic(
  ...
  [
    theoryStep(
      'numpy-aggregations-price-case',
      'Мини-кейс: признак `price`',
      'Смотрим числовой признак как часть EDA.',
      [
        section('case', 'Типичная цена, разброс и хвосты', [
          'Перед обучением модели почти всегда нужно понять, как устроен числовой признак: какие у него типичные значения, есть ли выбросы, насколько большой разброс. Поэтому в этой теме мы не просто считаем `mean` и `std`, а учимся читать признак как часть датасета.',
          'Допустим, у нас есть цены квартир. Перед моделью нужно быстро ответить: какая цена типичная, есть ли слишком дорогие объекты, насколько значения разбросаны и где начинаются хвосты распределения.',
        ], {
          table: {
            headers: ['Вопрос EDA', 'Функции'],
            rows: [
              ['какой общий масштаб', '`sum`, `min`, `max`'],
              ['где центр', '`mean`, `median`'],
              ['какой разброс', '`std`, `var`'],
              ['что в хвостах', '`quantile`, `percentile`'],
            ],
          },
        }),
      ],
    ...
    theoryStep(
      'numpy-aggregations-checkpoint',
      'Что теперь умеем',
      'Связываем статистики в один маршрут анализа.',
      [
        section('checkpoint', 'Промежуточный вывод', [
          'Теперь видно, что среднее и медиана отвечают на похожий вопрос, но ведут себя по-разному при выбросах. Поэтому в реальном EDA их часто смотрят вместе, а затем добавляют `std` и квантили.',
          'После группы функций у нас есть короткий маршрут: проверить границы, сравнить центр, оценить разброс, посмотреть хвосты распределения и только потом делать вывод о признаке.',
        ], {
          bullets: [
            'быстро оценивать центр распределения;',
            'отличать среднее от медианы;',
            'смотреть разброс через `std`;',
            'использовать квантили для хвостов распределения;',
            'считать статистики по признакам через `axis=0`.',
          ],
          callouts: [
            callout('Где это используется в ML/EDA', 'Первичный анализ числовых признаков, поиск выбросов, выбор стратегии заполнения пропусков, масштабирование признаков и проверка ошибок модели.', 'example'),
            callout('Типичная ошибка', 'Не стоит делать вывод о признаке только по среднему: один выброс может сильно сместить `mean`.', 'important'),
          ],
        }),
      ],
    ),
...
export const topicNumpyAxis = numpyTopic(
  ...
  [
    ...
    theoryStep(
      'numpy-2d-axis-checkpoint',
      'Axis без сухого запоминания',
      'Объясняем оси через матрицу объекты x признаки.',
      [
        section('axis-checkpoint', 'Что теперь умеем', [
          'В матрице `объекты x признаки` ось выбирают по тому, что должно остаться в результате. `axis=0` сворачивает строки и оставляет статистику по столбцам-признакам. `axis=1` сворачивает столбцы и оставляет статистику по строкам-объектам.',
          'После `reshape`, `.T` и `flatten()` важно проверять форму результата. Эти операции не просто “переставляют скобки”: они меняют то, как следующий код будет читать объекты и признаки.',
        ], {
          bullets: [
            '`axis=0` -> статистика по столбцам/признакам;',
            '`axis=1` -> статистика по строкам/объектам;',
            '`reshape()` меняет форму при том же числе элементов;',
            '`.T` меняет строки и столбцы местами;',
            '`flatten()` делает плоский вектор.',
          ],
          callouts: [
            callout('Типичная ошибка', 'Выбрать `axis=1` для статистики признаков и получить по одному числу на объект. Код может выполниться, но ответ будет про другой вопрос.', 'important'),
          ],
        }),
      ],
    ),
