...
export const topicNumpyMasks = numpyTopic(
  ...
  [
    ...
    theoryStep(
      ...
      'Закрепляем фильтрацию, замену и проверки.',
      [
        section('checkpoint', 'Что теперь умеем', [
          'Маски нужны, когда мы хотим оставить только подходящие объекты: дорогие квартиры, клиентов старше 30, значения без выбросов или строки в допустимом диапазоне. `x[mask]` уменьшает массив, а `np.where()` сохраняет форму и заменяет значения по правилу.',
          'В EDA это используется для проверки выбросов, фильтрации некорректных значений, создания бинарных признаков и быстрых sanity-checks через `np.any()` и `np.all()`.',
        ], {
          bullets: [
            'строить маску через сравнение;',
            'фильтровать массив записью `x[mask]`;',
            'заменять значения через `np.where()`;',
            'проверять маску целиком через `np.any()` и `np.all()`.',
            'Типичная ошибка: использовать обычные `and`, `or`, `not` вместо `&`, `|`, `~` и забывать скобки вокруг сравнений.',
          ],
        }),
      ],
    ),
...
export const topicNumpyRandom = numpyTopic(
  ...
  [
    ...
    theoryStep(
      ...
      'Связываем random с воспроизводимым ML-экспериментом.',
      [
        section('checkpoint', 'Что теперь умеем', [
          'Случайность нужна в ML, но эксперимент должен повторяться. Поэтому мы создаём `rng = np.random.default_rng(seed)` и дальше используем методы генератора для разбиения, перемешивания, bootstrap-подвыборок и синтетических данных.',
          'Одна и та же модель при разных случайных split может показать разное качество. Фиксация seed помогает сравнивать изменения кода, а не случайно удачный тестовый набор.',
        ], {
          bullets: [
            'создавать генератор через `default_rng(seed)`;',
            'генерировать целые, нормальные и равномерные значения;',
            'перемешивать порядок объектов через `permutation`;',
            'выбирать случайные элементы через `choice`.',
            'Типичная ошибка: сравнивать модели на разных случайных разбиениях и принимать случайно удачный test за улучшение алгоритма.',
          ],
        }),
      ],
    ),
