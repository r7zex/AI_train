  ...
  'ml-problem-types': [
    {
      ...
      alt: 'Две координатные плоскости сравнивают почти линейно расположенные точки с прямой и коэффициентом корреляции 0,96 слева с кольцевым расположением точек и коэффициентом корреляции около нуля справа.',
      caption: 'Что показано: слева сильная линейная связь, которую хорошо описывают прямая и высокий коэффициент корреляции; справа выраженная кольцевая структура при корреляции около нуля. Как читать: сравните форму облака точек и значение r. Главный вывод: низкая линейная корреляция не означает отсутствия закономерности — для кольцевой структуры нужны методы, способные учитывать нелинейные зависимости, в том числе методы машинного обучения.',
      placement: { stepId: 'ml-problem-types-theory', sectionId: 'what-ml-solves' },
      order: 1,
      provenance: { kind: 'generated', source: 'manual Matplotlib generation for topic 4.1 linear and nonlinear structure comparison' },
    },
  ],
  'validation-split': [
    {
